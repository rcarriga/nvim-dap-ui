...
return {
  ...
  setup_buffer = function(buf)
    require("dapui.render.loop").register_listener(
      "watches_modifiable", name, "render",
      function(b) vim.api.nvim_buf_set_option(b, "modifiable", true) end
    )
    api.nvim_buf_attach(
      buf, false, {
        on_lines = function(b)
  ...
  render = function(render_state) watches:render(render_state) end,
}
