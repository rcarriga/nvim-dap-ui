...
function BufBreakpoints:render(render_state, buffer, breakpoints, current_line,
  current_file, indent)
  ...
  for _, bp in pairs(breakpoints) do
    ...
    if vim.tbl_count(text) ~= 0 then
      ...
      render_state:add_match(is_current_line(bp) and
                               "DapUIBreakpointsCurrentLine" or
                               "DapUIBreakpointsLine", line_no, indent + 1,
                             #tostring(bp.line))
      ...
      local function add_info(message, data)
        ...
        render_state:add_match("DapUIBreakpointsInfo", render_state:length(),
                               info_indent, #message)
        render_state:add_mapping(config.actions.OPEN, open_frame_callback(bp))
      end
...
local function new() return BufBreakpoints:new() end
