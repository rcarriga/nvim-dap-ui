...
---@field expand_lines boolean Expand current line to hover window if larger than window size
---@field layouts dapui.Config.layout[] Layouts to display elements within. Layouts are opened in the order defined
...
---@field render dapui.Config.render Rendering options which can be updated after initial setup
...
---@field elements string[]|dapui.Config.layout.element[] Elements to display in this layout
---@field size number Size of the layout in lines/columns
---@field position "left"|"right"|"top"|"bottom" Which side of editor to open layout on
---
---@class dapui.Config.layout.element
---@field id string Element ID
---@field size number Size of the element in lines/columns or as proportion of total editor size (0-1)

---@class dapui.Config.floating
---@field max_height? number Maximum height of floating window (integer or float between 0 and 1)
---@field max_width? number Maximum width of floating window (integer or float between 0 and 1)
---@field border string|string[] Border argument supplied to `nvim_open_win`
---@field mappings table<dapui.FloatingAction, string|string[]> Keys to trigger actions in elements
...
---@field max_type_length? integer Maximum number of characters to allow a type name to fill before trimming
---@field max_value_lines? integer Maximum number of lines to allow a value to fill before trimming
