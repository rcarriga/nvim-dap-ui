...
function UIState:_clear()
  ...
  for _, receiver in pairs(self._listeners[events.CLEAR]) do receiver() end
end

function UIState:attach(dap, listener_id)
  listener_id = listener_id or "dapui_state"
  self._listener_id = listener_id
  dap.listeners.after.event_terminated[listener_id] =
...
function UIState:stop_monitor(var_ref)
  ...
  if self._monitored_vars[var_ref] then
    ...
    util.with_session(function(session) self:_emit_refreshed(session) end)
  end
end

function UIState:scopes() return self._scopes or {} end

function UIState:variables(ref) return self._variables[ref] end

function UIState:threads() return self._threads or {} end

function UIState:stopped_thread()
  return self:threads()[self._stopped_thread_id or -1]
end

function UIState:frames(thread_id) return self._frames[thread_id] or {} end

function UIState:current_frame() return self._current_frame end
...
function UIState:add_watch(expression, context)
  self._watches[expression] = self._watches[expression] or
                                {watchers = 0, context = context or "watch"}
  self._watches[expression].watchers = self._watches[expression].watchers + 1
  util.with_session(function(session) self:_refresh_watches(session) end)
end

function UIState:remove_watch(expression)
  if not self._watches[expression] then return end
  self._watches[expression].watchers = self._watches[expression].watchers - 1
...
function UIState:watches() return self._watches end

function UIState:watch(expression) return self._watches[expression] end
...
function UIState:on_clear(callback) self:_add_listener(events.CLEAR, callback) end

function UIState:refresh()
  util.with_session(function(session) self:_refresh(session) end)
end

---@return UIState
return function() return UIState:new() end
